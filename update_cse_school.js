...
async function updateCSE() {
    console.log("Fetching Computer Science professors...");
    const { data: profs, error } = await db.from('professors')
        .select('id, name, department, school')
        .ilike('department', '%Computer Science%');
    ...
    console.log(`Found ${profs.length} professors in Computer Science.`);
    ...
    for (let p of profs) {
        if (p.school !== "School of Computer Engineering") {
            ...
            if (updErr) {
                ...
                console.log(`[UPDATED] ${p.name} | Dept: ${p.department} -> School of Computer Engineering`);
                updatedCount++;
            }
        }
    }
