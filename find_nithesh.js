...
async function findNithesh() {
    let { data } = await db.from('professors').select('id, name').ilike('name', '%Nithesh%');
    console.log("Nithesh matches:", data ? data.map(d => d.name) : 'none');
    
    let { data: data2 } = await db.from('professors').select('id, name').ilike('name', '%Nitesh%');
    console.log("Nitesh matches:", data2 ? data2.map(d => d.name) : 'none');
}
