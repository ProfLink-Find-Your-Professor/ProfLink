...
async function dumpPending() {
    let { data: allProfs } = await db.from('professors').select('id, name').ilike('school', '%Electrical%');
    console.log("Electrical Engineering names:");
    allProfs.forEach(p => console.log(p.name));
}
dumpPending();
